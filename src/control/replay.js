...
let replay = function () {
  ...
  if (centerBtn.type === 'img') {
    btn = util.createDom('hk-replay', '<div class="hkplayer-replay-img"></div>', {}, 'hkplayer-replay')
    img = btn.querySelector('.hkplayer-replay-img')
    ...
    btn = util.createDom('hk-replay', `
          <svg class="hkplayer-replay-svg" xmlns="http://www.w3.org/2000/svg" width="78" height="78" viewbox="0 0 78 78">
            <path d="${iconPath.replay}"></path>
          </svg>
          <hk-replay-txt class="hkplayer-replay-txt">重播</hk-replay-txt>
          `, {}, 'hkplayer-replay')
    svg = btn.querySelector('.hkplayer-replay-svg')
  }
  ...
  function endedFunc () {
    if (centerBtn.type === 'img') {
      ...
      btn.querySelector('.hkplayer-replay-txt').textContent = player.lang.REPLAY
