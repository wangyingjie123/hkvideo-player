...
class Proxy {
    constructor(options) {
        ...
        this.videoConfig = {
            ...
            'webkit-playsinline': options.playsinline || true,
            'x5-playsinline': options.playsinline || true,
            'x5-video-player-type': options['x5-video-player-type'] || options['x5VideoPlayerType'] || 'h5',
        ...
        this.ev.forEach(item => {
            ...
            self.video.addEventListener(Object.keys(item)[0], function () {
                ...
                if (name === 'play') {
                    ...
                    util.removeClass(self.root, 'hkplayer-is-enter');
    ...
    set poster(posterUrl) {
        let poster = util.findDom(this.root, '.hkplayer-poster');
        if (poster) {
    ...
    get bullet() {
        return util.findDom(this.root, 'hk-danmu') ? util.hasClass(util.findDom(this.root, 'hk-danmu'), 'hkplayer-has-danmu') : false;
    }
};
